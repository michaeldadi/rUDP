...
while inputs:
    ...
    for s in readable:
        ...
        if data:
            ...
            recv_seq_num = struct.unpack_from('B', data)[0]
            # Packet ACK number
            recv_ack_num = struct.unpack_from('BB', data)[1]
            # Data payload length
            payload_len = struct.unpack_from('BBB', data)[2]
            # Data payload
            msg = struct.unpack_from('BBB%ds' % payload_len, data)[3]
    ...
    for s in writable:
        ...
        # Save data to file
        print(msg_queue.dequeue().decode())
